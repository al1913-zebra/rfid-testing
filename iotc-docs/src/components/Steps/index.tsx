    ...
    return (
        <div className={clsx(styles.steps)}>
            {(React.Children.toArray(children)
                .filter((child): child is React.ReactElement<{ children?: ReactNode }> => React.isValidElement(child) && child.type === Step)
                .map((child, index) => {
                    const childNodes = React.Children.toArray(child.props.children);
                    const titleNode = childNodes.find((c): c is React.ReactElement => React.isValidElement(c) && c.type === Title);
                    const content = childNodes.filter(c => !(React.isValidElement(c) && c.type === Title));
                    return titleNode ? (
